...
exports.getReliabilityMessage = getReliabilityMessage;
 ...
 */
function analyzeResponseSpeed(answers) {
    const flags = [];
    ...
    if (q1 && q2) {
        if (q1.selectedLevel >= 8 && q2.selectedLevel <= 4) {
            contradictions.push({
                type: 'contradiction',
                severity: 'high',
                message: 'Противоречие: говорите, что выбрали партнера из любви (высокий уровень), но постоянно пытаетесь его переделать (низкий уровень). Это признак условной любви.',
    ...
    if (q3 && q4) {
        if (q3.selectedLevel >= 8 && q4.selectedLevel <= 5) {
            contradictions.push({
                type: 'contradiction',
                severity: 'high',
                message: 'Противоречие: утверждаете полную аутентичность, но на чувствительных вопросах выбираете социально желательные ответы. Реальная уязвимость ниже заявленной.',
    ...
    if (q5 && q6) {
        if (q5.selectedLevel >= 9 && q6.selectedLevel <= 2) {
            contradictions.push({
                type: 'contradiction',
                severity: 'medium',
                message: 'Противоречие: утверждаете взаимный рост, но ничего не менялось за год. Возможно, идеализация или отрицание застоя.',
    ...
    if (q7 && q8) {
        if (q7.selectedLevel >= 10 && q8.selectedLevel <= 4) {
            contradictions.push({
                type: 'contradiction',
                severity: 'high',
                message: 'Противоречие: видите отношения как очень духовные, но при конфликте используете уход или защиту. Возможен духовный байпас - использование духовности чтобы избежать реальной работы.',
    ...
    if (q9 && q10) {
        if (q9.selectedLevel >= 9 && q10.selectedLevel <= 5) {
            contradictions.push({
                type: 'contradiction',
                severity: 'medium',
                message: 'Противоречие: утверждаете полную свободу и доверие, но ревность вас не отпускает. Это признак незавершенной работы над привязанностью.',
 ...
 */
function calculateCoherence(answers) {
    const flags = [];
    // Сгруппировать ответы по категориям
    const questionMap = new Map();
    questions_database_1.QUESTIONS.forEach((q) => questionMap.set(q.id, q));
    const categoryLevels = new Map();
    ...
    categoryLevels.forEach((levels, category) => {
        ...
        const variance = levels.reduce((sum, level) => sum + Math.pow(level - avg, 2), 0) /
            levels.length;
    ...
    if (overallCoherence < 40) {
        flags.push({
            type: 'incoherence',
            severity: 'high',
            message: 'Низкая когерентность: ваши ответы в одной категории сильно разнятся. Возможно, непредсказуемо, как вы себя ведете, или трудно определить стабильный уровень.',
        });
    }
    return {
 ...
 */
function detectSpiritualBypass(answers) {
    const flags = [];
    ...
    if (q1 && q1.selectedLevel >= 10) {
        ...
        if (q2 && q2.selectedLevel <= 4) {
            ...
            flags.push({
                ...
                severity: 'high',
                message: 'Возможен духовный байпас: видите отношения как священные, но на практике избегаете конфликтов или защищаетесь. Духовность может использоваться чтобы избежать реальной работы.',
            });
        }
    }
    ...
    if (q4 && q4.selectedLevel >= 10) {
        if (q3 && q3.selectedLevel <= 7) {
            ...
            flags.push({
                type: 'spiritual-bypass',
                severity: 'medium',
                message: 'Возможен духовный байпас: вы видите себя как духовного, но партнер не соответствует вашим идеалам. Это может быть духовным нарциссизмом.',
            });
        }
    }
    ...
    if (q6 && q6.selectedLevel >= 10) {
        if (q5 && q5.selectedLevel >= 2 && q5.selectedLevel <= 4) {
            ...
            flags.push({
                type: 'spiritual-bypass',
                severity: 'medium',
                message: 'Возможен духовный байпас: видите отношения как служение, но при этом отказываетесь от своих мечт. Истинное служение не требует полного самопожертвования.',
            });
        }
    }
    ...
    if (q8 && q8.selectedLevel >= 11) {
        ...
        const avgLevel = answers.reduce((sum, a) => sum + a.selectedLevel, 0) / answers.length;
        if (avgLevel <= 7) {
            bypassIndicators++;
            flags.push({
                type: 'spiritual-bypass',
                severity: 'high',
                message: 'Противоречие: утверждаете, что отношения на уровне Духовного союза, но в остальном ответе показываете более низкие уровни зрелости. Это может быть идеализация или духовный байпас.',
            });
        }
    }
    ...
    if (q9 && q9.selectedLevel >= 11 && q10 && q10.selectedLevel <= 5) {
        ...
        flags.push({
            type: 'spiritual-bypass',
            severity: 'high',
            message: 'Возможен духовный байпас: заявляете о высокой духовности, но на валидационных вопросах выбираете социально желательные ответы. Истинная духовность предполагает полную честность.',
        });
    }
 ...
 */
function validateTestResults(answers) {
    const warnings = [];
    ...
    if (speedAnalysis.anomaly) {
        recommendations.push('Старайтесь дольше размышлять перед ответом, особенно на сложные вопросы');
    }
    if (contradictionAnalysis.contradictions.length > 0) {
        recommendations.push('Обратите внимание на обнаруженные противоречия - они могут скрывать нерешённые психологические конфликты');
    }
    if (coherenceAnalysis.coherenceScore < 50) {
        recommendations.push('Ваши ответы в одной категории сильно варьируются - может быть полезно работать над стабильностью поведения');
    }
    if (bypassAnalysis.spiritualBypassScore > 0.5) {
        recommendations.push('Обратите внимание на возможный духовный байпас - баланс между идеалом и практикой очень важен');
    }
    const metrics = {
 ...
 */
function getReliabilityMessage(metrics) {
