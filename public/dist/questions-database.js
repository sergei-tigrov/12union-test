...
exports.getQuestionsByTargetLevel = getQuestionsByTargetLevel;
exports.getValidationQuestions = getValidationQuestions;
exports.getCriticalQuestions = getCriticalQuestions;
exports.getZoningQuestions = getZoningQuestions;
exports.getRefinementQuestions = getRefinementQuestions;
exports.QUESTIONS = [
    // ============================================================================
...
function getQuestionById(id) {
    return exports.QUESTIONS.find((q) => q.id === id);
}
/**
 * Получить вопросы по категории
 */
function getQuestionsByCategory(category) {
    return exports.QUESTIONS.filter((q) => q.category === category);
}
/**
 * Получить вопросы по целевым уровням
 */
function getQuestionsByTargetLevel(level) {
    return exports.QUESTIONS.filter((q) => q.targetLevels.includes(level));
}
/**
 * Получить вопросы валидации
 */
function getValidationQuestions() {
    return exports.QUESTIONS.filter((q) => q.isValidation);
}
/**
 * Получить критичные вопросы (priority 1)
 */
function getCriticalQuestions() {
    return exports.QUESTIONS.filter((q) => q.priority === 1);
}
/**
 * Получить вопросы для фазы зонирования
 * (быстрое определение приблизительной зоны 1-4, 5-8 или 9-12)
 */
function getZoningQuestions() {
    return exports.QUESTIONS.filter((q) => ['zone-conflict-001', 'zone-safety-002', 'zone-growth-003', 'zone-intimacy-004', 'zone-choice-005', 'zone-difference-006'].includes(q.id));
}
/**
 * Получить вопросы для фазы уточнения
 */
function getRefinementQuestions() {
    return exports.QUESTIONS.filter((q) => q.id.startsWith('level-detail-') || q.id.startsWith('boundary-') || q.id.startsWith('autonomy-') || q.id.startsWith('understanding-') || q.id.startsWith('hope-'));
}
