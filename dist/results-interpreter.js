...
exports.interpretPairComparison = interpretPairComparison;
const levels_definitions_1 = require("./levels-definitions");
const action_library_1 = require("./action-library");
// ============================================================================
...
function interpretResult(result) {
    ...
    const levelDef = (0, levels_definitions_1.getLevelDefinition)(roundedLevel);
    if (!levelDef) {
    ...
    const mainInsight = generateMainInsight(roundedLevel, result.testMode, levelDef.shortDescription);
    const currentChallenge = generateChallenge(roundedLevel, result.testMode);
    const growthPath = generateGrowthPath(roundedLevel, result.testMode);
    const nextLevelDef = roundedLevel < 12 ? (0, levels_definitions_1.getLevelDefinition)(roundedLevel + 1) : null;
    const nextLevel = nextLevelDef ? generateNextLevelPreview(nextLevelDef) : undefined;
    // Генерировать рекомендации
    const actionPlan = (0, action_library_1.getActionPlan)(roundedLevel);
 ...
 */
function generateHeroMessage(level, mode, status) {
    const levelInt = Math.round(level);
    // Специфичные для режима
    if (mode === 'self' && status === 'single_past') {
        return `По результатам вашего анализа, вы находитесь на уровне ${levelInt} - ${(0, levels_definitions_1.getLevelDefinition)(levelInt)?.name}. Это уровень где вы в отношениях прошлого.`;
    }
    ...
    if (mode === 'self' && status === 'in_relationship') {
        return `В ваших текущих отношениях вы развили уровень ${levelInt} - ${(0, levels_definitions_1.getLevelDefinition)(levelInt)?.name}. Это ваша нынешняя реальность.`;
    }
    if (mode === 'partner_assessment') {
        return `По вашей оценке, ваш партнер находится на уровне ${levelInt} - ${(0, levels_definitions_1.getLevelDefinition)(levelInt)?.name}. Это то как вы его/её видите.`;
    }
    ...
    if (mode === 'pair_discussion') {
        return `Вместе вы создали союз уровня ${levelInt} - ${(0, levels_definitions_1.getLevelDefinition)(levelInt)?.name}. Это уровень вашей совместной зрелости.`;
    }
    return `Ваш уровень - ${levelInt}: ${(0, levels_definitions_1.getLevelDefinition)(levelInt)?.name}`;
}
/**
 * Генерировать главный инсайт
 */
function generateMainInsight(level, mode, shortDescription) {
    const levelInt = Math.round(level);
    if (levelInt <= 3) {
 ...
 */
function generateChallenge(level, mode) {
    const levelInt = Math.round(level);
    const challenges = {
 ...
 */
function generateGrowthPath(level, mode) {
    const levelInt = Math.round(level);
    if (levelInt <= 3) {
 ...
 */
function generateNextLevelPreview(nextLevelDef) {
    if (!nextLevelDef)
        return '';
...
function interpretPairComparison(result) {
    ...
    const agreementPoints = findAgreementPoints(result.personalLevel, result.comparisonWith?.personalLevel || 0);
    const conflictPoints = findConflictPoints(result.personalLevel, result.comparisonWith?.personalLevel || 0);
    // Генерировать рекомендации для пары
    const growthRecommendations = generatePairRecommendations(result.personalLevel, result.comparisonWith?.personalLevel || 0, gap);
    // Рассчитать совместимость
    const compatibilityScore = result.compatibility || calculateCompatibility(result.personalLevel, result.comparisonWith?.personalLevel || 0);
 ...
 */
function generateGapMessage(personalLevel, gap, mode) {
    if (Math.abs(gap) < 0.5) {
 ...
 */
function findAgreementPoints(level1, level2) {
    const agreements = [];
 ...
 */
function findConflictPoints(level1, level2) {
    const conflicts = [];
 ...
 * Генерировать рекомендации для пары
 */
function generatePairRecommendations(level1, level2, gap) {
    const recommendations = [];
    if (Math.abs(gap) >= 2) {
        recommendations.push('Работайте с парным терапевтом или коучем для моста между уровнями');
    }
 ...
 * Рассчитать совместимость
 */
function calculateCompatibility(level1, level2) {
    const gap = Math.abs(level1 - level2);
    let score = 100;
    // Штраф за расстояние
    score -= gap * 10;
...
// ============================================================================
function getTestModeContext(mode, status) {
    if (mode === 'self') {
        if (status === 'single_past')
            return 'анализ прошлых отношений';
        if (status === 'single_potential')
            return 'оценка своего потенциала';
        return 'самооценка в текущих отношениях';
    }
    if (mode === 'partner_assessment')
        return 'оценка партнера';
    if (mode === 'potential')
        return 'оценка потенциала';
    if (mode === 'pair_discussion')
        return 'совместное обсуждение';
    return 'оценка';
}
