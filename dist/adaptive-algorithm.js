...
exports.initializeAdaptiveTest = initializeAdaptiveTest;
exports.getNextQuestion = getNextQuestion;
exports.recordAnswer = recordAnswer;
exports.getCurrentLevelDetection = getCurrentLevelDetection;
 ...
 */
function initializeAdaptiveTest(sessionId) {
    return {
        sessionId,
        answers: [],
        currentPhase: 'zoning',
 ...
 */
function detectZone(zoneAnswers) {
    const levels = Array.from(zoneAnswers.values());
 ...
 */
function calculateLevelScores(answers, questions) {
    ...
    const totalScore = Array.from(levelScores.values()).reduce((sum, s) => sum + s.score, 0);
    if (totalScore > 0) {
 ...
 */
function detectCurrentLevel(answers, questions) {
    ...
    return {
        ...
        zone,
        levelScoreDistribution: new Map(scores.map(([level, { confidence }]) => [level, confidence])),
    };
}
/**
 * Выбрать следующий вопрос на основе адаптивной логики
 */
function selectNextQuestion(state, allQuestions) {
    const availableQuestions = Array.from(allQuestions.values()).filter((q) => !state.questionsAsked.has(q.id));
    ...
    if (state.currentPhase === 'refinement') {
        ...
        const refinementQuestions = availableQuestions.filter((q) => {
            ...
            const isRefinement = q.id.startsWith('level-detail-') ||
            ...
            const overlapsWithTarget = q.targetLevels.some((level) => targetLevelRange.includes(level));
    ...
    if (state.currentPhase === 'validation') {
        const validationQuestions = availableQuestions.filter((q) => q.isValidation === true);
...
function advancePhase(state) {
    if (state.currentPhase === 'zoning') {
        ...
        const zoningAnswered = state.answers.filter((a) => zoningQIds.includes(a.questionId)).length;
        if (zoningAnswered >= 6) {
...
function getNextQuestion(state) {
    ...
    questions_database_1.QUESTIONS.forEach((q) => questionsMap.set(q.id, q));
 ...
 */
function getCurrentLevelDetection(state) {
    const questionsMap = new Map();
    questions_database_1.QUESTIONS.forEach((q) => questionsMap.set(q.id, q));
 ...
 */
function createQuestionSelection(state, question, questionsMap) {
    const estimatedLevel = state.questionsAnswered > 0
