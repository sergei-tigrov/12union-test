 ...
 * Начать новый тест
 */
app.post('/api/test/initialize', (req: Request, res: Response) => {
  try {
    const { testMode = 'self', relationshipStatus = 'in_relationship' } = req.body;

    const context = API.initializeTestSession(
 ...
 */
app.get('/api/test/:sessionId/next-question', (req: Request, res: Response) => {
  try {
 ...
 * Отправить ответ
 */
app.post('/api/test/:sessionId/answer', (req: Request, res: Response) => {
  try {
    const { sessionId } = req.params;
 ...
 */
app.post('/api/test/:sessionId/complete', (req: Request, res: Response) => {
  try {
    const { sessionId } = req.params;
