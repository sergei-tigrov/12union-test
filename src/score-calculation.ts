...
export function calculateLevelScores(
  ...
  // Минимум 10 ответов (меньше этого диагноз будет крайне неточным)
  // Адаптивный тест обычно даёт 18-25 вопросов, но может завершиться раньше
  if (answers.length < 10) {
    throw new Error(
      `Недостаточно данных для расчета результатов. ` +
      `Получено ответов: ${answers.length}, требуется минимум 10. ` +
      `Пожалуйста, продолжите прохождение теста.`
    );
  }

  // Предупреждение при малом количестве данных (но не критичном)
  if (answers.length < 15) {
    console.warn(
      `⚠️ Внимание: малое количество ответов (${answers.length}). ` +
      `Рекомендуется минимум 15 для точной диагностики. ` +
